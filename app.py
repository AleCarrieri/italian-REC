                ...
                children=[
                    ...
                    dcc.Tabs(id="tabs_styled_with_props", value='Residential',
                             children=[
                                 ...
                                 dcc.Tab(label='Commercial', value='Commercial',
                                         style={'fontWeight': 'bold'},
                                         selected_style={
                                             'backgroundColor': 'steelblue',
                                             'color': 'white',
                                 ...
                                 dcc.Tab(label='Industrial', value='Industrial',
                                         style={'fontWeight': 'bold'},
                                         selected_style={
                                             'backgroundColor': 'steelblue',
                                             'color': 'white',
                             ...
                             ], colors={"border": "white","primary": "gold","background": "#2d3038"}
                            ),
        ...
        return dbc.Container([
              ...
              dcc.Tabs(
                  ...
                  children=[
                      dcc.Tab(
                          ...
                          value='tab-1',
                          style={'fontWeight': 'bold', 'color': '#2d3038'},
                          selected_style={
                              'backgroundColor': 'steelblue',
                              'color': 'white',
                          }
                  ...
                  ], colors={"border": "white","primary": "gold","background": "lightgrey"}),
              html.Br(),
          ...
          return dbc.Container([
            html.Div([dbc.Jumbotron(
              ...
              dbc.Row([
                  dbc.Col([
                      html.Label(['REC annual management cost [EUR]:'], style={'font-weight': 'bold', 'color': 'navy'}),
                      dcc.Input(id="REC_Cost_numeric_input", type="number", min=1, max=100, value=5),
                      html.Br(),
                      html.Br(),
                      html.Br(),
                      html.Br(),
                      html.Label(['Discount rate [%]:'], style={'font-weight': 'bold', 'color': 'navy'}),
                      dcc.Input(id="Discount_rate_numeric_input", type="number", min=1, max=100, value=5),
                      html.Br(),
                      html.Br(),
                      html.Br(),
                      html.Br(),
                      html.Label(['Tax rate for energy revenues [%]:'], style={'font-weight': 'bold', 'color': 'navy'}),
                      dcc.Input(id="Taxrate_numeric_input", type="number", min=1, max=100, value=20),
                  ]),
                  dbc.Col([
                      dbc.Card([
                          dbc.CardHeader(Lottie(options=options, width="50%", height="50%",
                  ...
                  dbc.Col([
                      ...
                      html.Br(),
                      html.Br(),
                      html.Label(['Legend position:'], style={'font-weight': 'bold', 'color': 'navy'}),
                      dbc.Row([
                          dbc.Col([
                              dbc.FormGroup(
                                  [
...
# callback to see Title member cons
@app.callback(Output('title_member_cons', 'children'),
              [Input('User', 'value')],
              )
def update_output(value):
    return [value.replace('Consumption', '')]

# callback to see Title member prod
@app.callback(Output('title_member_prod', 'children'),
              [Input('Prod', 'value')],
              )
def update_output(value):
    return [value.replace('Production', '')]
...
def update_values(rows, User, value):
    ...
    Energy_expenditure = round((Energy_fromthegrid * 150), 0)
    Savings=round((Energy_SelfCons* 150),0)
    ...
    GSE_Incentive = round((df_cons[Member + ' GSE Incentive'].sum()), 0)
...
def update_values(rows, Prod, value):
    ...
    SaleEnergy = round((Energy_intothegrid * 50.1), 0)
    ...
    GSE_Incentive = round((df_prod[Member + ' GSE Incentive'].sum()), 0)
        ...
        return html.Div([
            ...
            dbc.Row([
                dbc.Col([
                    ...
                    dbc.FormGroup(
                        [
                            html.Label(['Type of dwelling:'], style={'font-weight': 'bold'}),
                            html.Br(),
                            dbc.RadioItems(
                                id='typeofdwelling',
                                options=[
                                    {'label': 'Single family villa', 'value': 'Single family villa'},
                                    {'label': 'Multifamily villa', 'value': 'Multifamily villa'},
                                    {'label': 'Apartments in building with less than 10 apartments',
                                     'value': 'Apartments in building with less than 10 apartments'},
                                    {'label': 'Apartments in building with 10 or more apartments',
                                     'value': 'Apartments in building with 10 or more apartments'},
                                ],
                                value='Apartments in building with less than 10 apartments',
                            ),
                        ]
        ...
        return html.Div([
            ...
            dbc.Row([
                ...
                dbc.Col([
                    dbc.FormGroup(
                        [
                            html.Label(['Type of activity:'], style={'font-weight': 'bold', 'color': 'white'}),
                            html.Br(),
                            dbc.RadioItems(
                                id='typeactivity',
                                options=[
            ...
            dbc.Row([
                ...
                dbc.Col([
                    html.Div(id="content_commercial"),
                ],width=8),
            ]),
            dbc.Row([
                dcc.Dropdown(
...
@app.callback(
    dash.dependencies.Output('LED_display_Hotel', 'value'),
    [dash.dependencies.Input('value_Area_Res', 'value')]
)
def update_outputOff(nrooms):
    value=7350*nrooms
    return str(value)
...
@app.callback(
    ...
    dash.dependencies.Output('menu_res', 'value'),
    [dash.dependencies.Input('npeople_numeric_input', 'value')]
)
def update_options(npeople):
    if npeople==1:
        options=[
        ...
        value = "Family, 1 child, 1 at work, 1 at home"
        return options, value
    elif npeople==3:
        options=[
        ...
        value = "Family, 1 child, with work"
        return options, value
    else:
        options=[
        ...
        value = "Family, 2 children, both at work"
        return options, value
...
#Residential
def loadresidential(city, value_Area_Res, menu_res, typeofdwelling, dw_boolean_switch, aircond_boolean_switch, HP_boolean_switch, editing_columns_name):
    ...
    if (typeofdwelling == 'Single family villa'):
        df_LOAD[editing_columns_name] = df_LOAD[editing_columns_name] + (0.0836 * df_LOAD[editing_columns_name])
    elif (typeofdwelling == 'Multifamily villa'):
        df_LOAD[editing_columns_name] = df_LOAD[editing_columns_name] + (0.0818 * df_LOAD[editing_columns_name])
    elif (typeofdwelling == 'Apartments in building with less than 10 apartments'):
        df_LOAD[editing_columns_name] = df_LOAD[editing_columns_name]
    elif (typeofdwelling == 'Apartments in building with 10 or more apartments'):
        df_LOAD[editing_columns_name] = df_LOAD[editing_columns_name] + (-0.0611 * df_LOAD[editing_columns_name])



    if (dw_boolean_switch == True):
        df_LOAD[editing_columns_name] = df_LOAD[editing_columns_name] + (0.0818 * df_LOAD[editing_columns_name])

    if (aircond_boolean_switch == True):
...
def load_commercial(menu_comm, value_Area_Res, editing_columns_name):
    ...
    if (menu_comm == 'Shop open 7 days a week'):
        df_LOAD[editing_columns_name] = value_Area_Res * df_LOAD[editing_columns_name]
    if (menu_comm == 'Seasonal Hotel'):
        df_LOAD[editing_columns_name] = df_LOAD[editing_columns_name] * value_Area_Res * 7000
    if (menu_comm == 'Hotel Always open'):
        df_LOAD[editing_columns_name] = df_LOAD[editing_columns_name] * value_Area_Res * 7000
    return df_LOAD.round(decimals=3)

#ipotesi consumo hotel 7000 kWh a stanza  RSE/2009/162 ... Il consumo di energia elettrica varia, come valori minimo/massimo, da 5 a 11 MWh per stanza l'anno
...
def update_data(n_clicks, tab, city, value_Area_Res, menu_res, typeofdwelling, dw_boolean_switch, aircond_boolean_switch, HP_boolean_switch, menu_comm, menu_ind, editing_columns_name):
    changed_id = [p['prop_id'] for p in dash.callback_context.triggered][0]
    if 'submit-button' in changed_id:
        if tab == 'Residential':
            df_LOAD = loadresidential(city, value_Area_Res, menu_res, typeofdwelling, dw_boolean_switch, aircond_boolean_switch, HP_boolean_switch, editing_columns_name)
        if tab == 'Commercial':
...
def update_columns(n_clicks,  editing_columns_name, existing_columns):
    ...
    if 'submit-button' in changed_id:
        existing_columns.append({
            'id': editing_columns_name, 'name': editing_columns_name + ' [kW]', 'deletable': True
        }),
...
#callback for updating consumer content
@app.callback(
    Output('content_commercial', 'children'),
    Output('menu_comm', 'options'),
    Output('menu_comm', 'value'),
    [Input('typeactivity', 'value')])
def updategraphs(value):
    if value=="Office":
        ...
        content=[
            ...
            html.Br(),
            html.Br(),
            html.Label(['Number of rooms:'], style={'font-weight': 'bold', 'color': 'white'}),
            html.Br(),
            dcc.Input(id="input_circular_nrooms", type="number", min=10, max=300, value=50,
                      placeholder="Rooms"),
            html.Br(),
            html.Br(),
            dcc.Slider(
            ...
            html.Br(),
            daq.LEDDisplay(
                id='LED_display_Hotel',
                label="Yearly Consumption [kWh/y]",
                value=1,
        ...
        options = [
            {'label': 'Seasonal Hotel', 'value': 'Seasonal Hotel'},
            {'label': 'Hotel Always open', 'value': 'Hotel Always open'},
            ]
        value="Hotel Always open"
        return content, options, value
...
@app.callback(
    Output("input_circular_nrooms", "value"),
    Output("value_Area_Res", "value"),
    Input("input_circular_nrooms", "value"),
    Input("value_Area_Res", "value"),
)
def callback(input_value, slider_value):
    ctx = dash.callback_context
    trigger_id = ctx.triggered[0]["prop_id"].split(".")[0]
    value = input_value if trigger_id == "input_circular_nrooms" else slider_value
    return value, value
...
def update_columns(n_clicks,  editing_columns_name, existing_columns):
    ...
    if 'add_members' in changed_id:
        existing_columns.append({
            'id': editing_columns_name + " Consumption", 'name': editing_columns_name + " Consumption",  'deletable': True,
        }),
        existing_columns.append({
            'id': editing_columns_name + " Production", 'name': editing_columns_name + " Production",  'deletable': True,
        }),
        existing_columns.append({
            'id': editing_columns_name + " Real Self Cons", 'name': editing_columns_name + " Real Self Cons",  'deletable': True,
        }),
    return existing_columns
...
def update_values(rows):
    ...
    # calcolo il totale della comunitá (sommo le colonne) ottengo colonna TOT 8760 valori
    df_cons['EC Consumption [kW]'] = df_cons.iloc[:, 1:(len(df_cons.columns) - 7)].sum(axis=1)
    # calcolo il valore totale annuale in MWh
    EC_Cons = round((df_cons['EC Consumption [kW]'].sum()/1000), 0),
    ...
    df_prod['EC Production [kW]'] = df_prod.iloc[:, 1:(len(df_prod.columns) - 7)].sum(axis=1)
    # calcolo il valore totale annuale in MWh
    EC_Prod = round((df_prod['EC Production [kW]'].sum() / 1000), 0),
    ...
    Energy_fromthegrid = round((df_members['Energy withdrawn from the grid'].sum() / 1000), 0)
    ...
    Energy_intothegrid = round((df_members['Energy fed into the grid'].sum() / 1000), 0)
    SaleEnergy=round((Energy_intothegrid* 50.1),0)


    df_community['Shared electricity [kW]'] = df_members[['Energy fed into the grid', 'Energy withdrawn from the grid']].min(axis=1)
    Shared_electricity=round((df_community['Shared electricity [kW]'].sum() / 1000), 0)
    ...
    GSEInc = round(df_community['GSEInc'].sum(), 0),
    ...
    Energy_SelfCons = round((df_members['Real Self Cons[kW]'].sum() / 1000), 0)
    Savings=round((Energy_SelfCons* 150),0)

    # ISPRA- Istituto Superiore per la Protezione e la Ricerca Ambientale 2020 (276.4 g/kWh el)
    CO2Red=round((df_prod['EC Production [kW]'].sum() / 1000)*0.276, 0)
    ...
    if tab == 'tab-1':
        return html.Div([
            ...
            dbc.Collapse([
                ...
                html.Div([
                    html.Div([
                        html.B("Yearly EC Consumption [MWh/y]", style={'color': 'navy'}),
                    ],
                        className="two columns", style={'textAlign': 'center'}
                    ),
                    html.Div([
                        html.B("Yearly EC Production [MWh/y]", style={'color': 'navy'}),
                    ],
                        className="two columns", style={'textAlign': 'center'}
                    ),
                    html.Div([
                        html.B("Yearly Real Self Consumption [MWh/y]", style={'color': 'navy'})
                    ],
                        className="two columns", style={'textAlign': 'center'}
                    ),
                    html.Div([
                        html.B("Yearly Shared energy [MWh/y]", style={'color': 'navy'})
                    ],
                        className="two columns", style={'textAlign': 'center'}
                    ),
                    html.Div([
                        html.B("Energy fed into the grid [MWh/y]", style={'color': 'navy'}),
                    ],
                        className="two columns", style={'textAlign': 'center'}
                    ),
                    html.Div([
                        html.B("Energy withdrawn from the grid [MWh/y]", style={'color': 'navy'})
                    ],
                        className="two columns", style={'textAlign': 'center'}
                    ),
                ],
                ...
                html.Div([
                    ...
                    html.Div([
                        dbc.Card([
                            dbc.CardHeader(
                                Lottie(options=options, width="100%", height="100%", url="assets/77596-save-energy.json")),
                            dbc.CardBody([
                                daq.LEDDisplay(
                                    id='ingrid_LED',
                ...
                html.Hr(),
                dbc.Row([
                    html.H3(["Change LED size"],
                            style={'font-style': 'italic', 'font-weight': 'bold', 'color': 'navy'}),
                    html.Br(),
                ], justify='center'),
                dcc.Slider(
                    id="slider_size_comm", min=20, max=45,
                    value=32
                ),
                html.Br(),
                html.Div([
                    html.Div([
                        html.B("Yearly GSE Incentive [EUR]", style={'color': 'navy'}),
                    ],
                        className="two columns", style={'textAlign': 'center'}
                    ),
                    html.Div([
                        html.B("Yearly Revenue from energy sold to the grid [EUR]", style={'color': 'navy'}),
                    ],
                        className="two columns", style={'textAlign': 'center'}
                    ),
                    html.Div([
                        html.B("Savings for non-purchased energy [EUR]", style={'color': 'navy'})
                    ],
                        className="two columns", style={'textAlign': 'center'}
                    ),
                    html.Div([
                        html.B("Annual Carbon dioxide avoided [tCO2/year]", style={'color': 'navy'})
                    ],
                        className="two columns", style={'textAlign': 'center'}
                    ),
                ],
                ...
                html.Div([
                    ...
                    html.Div([
                        dbc.Card([
                            dbc.CardHeader(
                                Lottie(options=options, width="100%", height="100%", url="assets/69847-pulsing-sun.json")),
                            dbc.CardBody([
                                daq.LEDDisplay(
                                    id='CO2Red_LED',
    ...
    if tab == 'tab-2':
        return html.Div([
            ...
            dbc.Collapse([
                ...
                html.Br(),
                html.Br(),
                html.H1(id='title_member_cons',
                        style={'color': 'navy', 'textAlign': 'center', 'font-style': 'italic', 'font-weight': 'bold'}),
                html.Br(),
                html.Br(),
                html.Div([
                    html.Div([
                        html.B("Yearly Consumption [MWh/y]", style={'color': 'navy'}),
                    ],
                        className="two columns", style={'textAlign': 'center'}
                    ),
                    html.Div([
                    ...
                    html.Div([
                        html.B("Annual Energy expenditure [EUR]", style={'color': 'navy'})
                    ],
                        className="two columns", style={'textAlign': 'center'}
                    ),
                    html.Div([
                ...
                dcc.Slider(
                    id="slider_size_cons", min=20, max=45,
                    value=32
                ),
                html.Br(),
                dbc.Row([
            ...
            dbc.Collapse([
                ...
                dcc.Slider(
                    id="slider_size_prod", min=20, max=45,
                    value=32
                ),
                html.Br(),
                dbc.Row([
...
# _____________________________________________________________________________________________________________________
#callbacks to change daq size
@app.callback(
    Output("CO2Red_LED", 'size'),
    [Input("slider_size_comm", 'value')],
)
def updatesize(value):
    return value

@app.callback(
    Output("Savings_LED", 'size'),
    [Input("slider_size_comm", 'value')],
)
def updatesize(value):
    return value
...
@app.callback(
    Output("Shared_electricity_LED", 'size'),
    [Input("slider_size_comm", 'value')],
)
def updatesize(value):
    return value
...
@app.callback(
    Output("EC_Consumption_LED", 'size'),
    [Input("slider_size_comm", 'value')],
)
def updatesize(value):
    return value
...
@app.callback(
    Output("EC_Consumption_member_LED", 'size'),
    [Input("slider_size_cons", 'value')],
)
def updatesize(value):
    return value

@app.callback(
    Output("RealSelf_member_LED", 'size'),
    [Input("slider_size_cons", 'value')],
)
def updatesize(value):
    return value
...
@app.callback(
    Output("fromgrid_member_LED", 'size'),
    [Input("slider_size_cons", 'value')],
)
def updatesize(value):
    return value
...
@app.callback(
    Output("Energy_expenditure_member_LED", 'size'),
    [Input("slider_size_cons", 'value')],
)
def updatesize(value):
    return value
...
@app.callback(
    Output("Savings_member_LED", 'size'),
    [Input("slider_size_cons", 'value')],
)
def updatesize(value):
    return value
...
@app.callback(
    Output("GSE_Incentive_member1_LED", 'size'),
    [Input("slider_size_cons", 'value')],
)
def updatesize(value):
    return value
...
@app.callback(
    Output("EC_Production_member_LED", 'size'),
    [Input("slider_size_prod", 'value')],
)
def updatesize(value):
    return value

@app.callback(
    Output("ingrid_member_LED", 'size'),
    [Input("slider_size_prod", 'value')],
)
def updatesize(value):
    return value
...
@app.callback(
    Output("SaleEnergy_member_LED", 'size'),
    [Input("slider_size_prod", 'value')],
)
def updatesize(value):
    return value
...
@app.callback(
    Output("GSE_Incentive_member2_LED", 'size'),
    [Input("slider_size_prod", 'value')],
)
def updatesize(value):
    return value
...
@app.callback(
    ...
    [Input('add_members', 'n_clicks'),
     Input('datatable_prod-summary', 'data'),
     Input('member_type', 'value'),
     Input('name', 'value'),
     Input('dropdown_members_cons', 'value'),
     Input('dropdown_members_prod', 'value'),
...
def ECsummary(n_clicks, data, value, name, load, prod, perc, detr, selfcons, CostoEnergia, rows):
    df_PVsummary = pd.DataFrame(data)
    changed_id = [p['prop_id'] for p in dash.callback_context.triggered][0]
    if 'add_members' in changed_id:
        if value == "Consumer":
            a = {
            ...
            info_Potenza = df_PVsummary[df_PVsummary['Name'] == prod]
            # Potenza_installata impianto reale[kWp]
            x3 = info_Potenza.iloc[0]['PV Nominal Power [kW]']
            if pd.isna(x3) == True:
                Potenza_installata = 0
            else:
                Potenza_installata = (info_Potenza.iloc[0]['PV Nominal Power [kW]'])

            # Costospecifico_(impianto + installazione) [€/kWp]
            if Potenza_installata <= 1.55:
                # EPC = 1667 * (1 + 0.08)
                # TPC = EPC * (1 + 0.2)
                # Costospecifico = TPC * (1 + 0.2)
                Costospecifico = 1667
            elif 1.55 < Potenza_installata <= 3:
                # EPC = 2000 * (1 + 0.08)
                # TPC = EPC * (1 + 0.2)
                # Costospecifico = TPC * (1 + 0.2)
                Costospecifico = 2000
            elif 3 < Potenza_installata <= 4.5:
                Costospecifico = 1889
            elif 4.5 < Potenza_installata <= 6:
                Costospecifico = 1717
            elif 6 < Potenza_installata <= 10:
                Costospecifico = 1850
            elif 10 < Potenza_installata <= 20:
                Costospecifico = 1600
            elif 20 < Potenza_installata <= 1000:
        ...
        rows.append(a)
        columns = [
            {"name": "User_ID", 'id': 'User_ID', 'editable': 'False'},
            {"name": "Load", 'id': 'Load', 'editable': 'False'},
            {"name": "PVPlant", 'id': 'PVPlant', 'editable': 'False'},
            {"name": "% PV", 'id': '% PV', 'editable': 'False'},
            {"name": "User_Type", 'id': 'User_Type', 'editable': 'False'},
            {"name": "Bonus 50%", 'id': 'Bonus 50%'},
            {"name": "Real Self Consumption", 'id': 'Real Self Consumption', 'editable': 'False'},
...
@app.callback(Output('economics_graph', 'figure'),
              [Input('dropdown_member', 'value'),
               ...
               Input("xanchor", "value"),
               Input("yanchor", "value"),
               Input("REC_Cost_numeric_input", "value"),
               Input('Taxrate_numeric_input', 'value'),
               Input('Discount_rate_numeric_input', 'value'),

               ]
              )
def create_graph(Member, rows_data, rows_summary, data, value, pos_x, pos_y, REC_Cost, Taxrate, Discount_rate):
    n_years = 20
...
#____________________________________________________________________
    ...
    # Nome impianto PV
    x2 = info_member.iloc[0]['PVPlant']
    if pd.isna(x2) == True:
        NamePV = []
    else:
        NamePV = (info_member.iloc[0]['PVPlant'])

    if NamePV == []:
        Potenza_installata = 0
    else:
        info_Potenza = df_PVsummary[df_PVsummary['Name'] == NamePV]
        Potenza_installata = (info_Potenza.iloc[0]['PV Nominal Power [kW]'])
...
# Costo_investimento del membro della comunitá[€]
    ...
    Costo_investimento = Costospecifico * Potenza_installata * Percentuale_proprietà
    ...
    Costo_OM = Potenza_installata * CostospecificoOM * Percentuale_proprietà


    # ______________________________________________________________________________
    # PREZZO ENERGIA ELETTRICA VENDUTA ALLA RETE [€/MWh]  50.0	€/MWh
    Prezzo_venditaRete=50
    ...
    # Corrispettivo GSE produzione Corrispettivo fisso €/anno & Corrispettivo variabile €/kW
    if Potenza_installata < 3:
        Corrispettivofisso = 0
        Corrispettivo_variabile = 0
    elif 3 <= Potenza_installata <= 20:
        Corrispettivofisso = 30
        Corrispettivo_variabile = 0
    elif Potenza_installata > 20:
        Corrispettivofisso = 30
        Corrispettivo_variabile = 1 * Potenza_installata

    # Contributo aggiuntivo di € 4,00 / annuo per ogni punto di connessione
    Contributo_aggiuntivo = 4

    SpeseGSE = ((Corrispettivofisso * Percentuale_proprietà) + (Corrispettivo_variabile * Percentuale_proprietà)) + Contributo_aggiuntivo
    ...
    SpeseGSE = (-SpeseGSE)
    speseGSE = [SpeseGSE] * (n_years + 1)
    speseGSE[0] = 0



    zipped_lists = zip(ric, amm, detrtot, oem, speseGSE)
    ris = [x + y + z + i + k for (x, y, z, i, k) in zipped_lists]

    aliquota_imposte = Taxrate/100
    imp = [- x * aliquota_imposte for x in riceneprod]

    CostoPartecipazione = -REC_Cost
    costo = [CostoPartecipazione] * (n_years + 1)
    costo[0] = 0

    df_CASHFLOW = pd.DataFrame(list(zip(index, inv, riceneprod, rispself, ricGSE, ric, amm, detrtot, oem, speseGSE, ris, imp, costo)),
                               columns=['Anno', 'Investimento', 'Ricavi energia venduta alla rete',
                                        'Risparmi per autoconsumo', 'Ricavi incentivo GSE', 'Ricavi totali',
                                        'Ammortamento', 'Detrazioni', 'O&M', 'Corrispettivo GSE','Risultato pre imposte', 'Imposte','Costo partecipazione comunitá'])

    df_CASHFLOW['Flusso di cassa netto'] = df_CASHFLOW['Ricavi totali']+ df_CASHFLOW['Detrazioni'] + df_CASHFLOW['O&M'] + df_CASHFLOW['Imposte'] + df_CASHFLOW['Costo partecipazione comunitá'] + df_CASHFLOW['Corrispettivo GSE']

    df_CASHFLOW.at[0, 'Flusso di cassa netto'] = -(Costo_investimento)

    Tassodisconto_i = Discount_rate/100
    ...
    economics_graph=go.Figure(
        data=[
            ...
            go.Bar(
                x=df_CASHFLOW["Anno"],
                y=df_CASHFLOW["Corrispettivo GSE"],
                name="Corrispettivo GSE",
